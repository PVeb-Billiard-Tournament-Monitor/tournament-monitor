...
-- субота, 21. јануар 2017. 22:23:05 CET
...
CREATE TABLE IF NOT EXISTS `billiard_db`.`match` (
  ...
  `table_id` TINYINT NULL DEFAULT NULL,
  `winner_id` INT NULL,
  `tournament_date` DATE NOT NULL,
  `billiard_club_id` INT UNSIGNED NOT NULL,
  `tournament_type` VARCHAR(45) NOT NULL,
  `phony` TINYINT(1) NOT NULL DEFAULT 0,
...
-- -----------------------------------------------------
-- Table `billiard_db`.`new_match`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `billiard_db`.`new_match` ;

CREATE TABLE IF NOT EXISTS `billiard_db`.`new_match` (
  `match_id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `tournament_date` DATE NOT NULL,
  `billiard_club_id` INT UNSIGNED NOT NULL,
  `tournament_type` VARCHAR(45) NOT NULL,
  `id_player1` INT NULL DEFAULT NULL,
  `id_player2` INT NULL DEFAULT NULL,
  `score1` VARCHAR(45) NOT NULL DEFAULT 0,
  `score2` VARCHAR(45) NOT NULL DEFAULT 0,
  `round` INT NOT NULL DEFAULT 1,
  `finished` TINYINT(1) NOT NULL DEFAULT 0,
  `at_table` TINYINT(1) NOT NULL DEFAULT 0,
  `winner_id` INT NULL DEFAULT NULL,
  PRIMARY KEY (`match_id`, `tournament_date`, `billiard_club_id`, `tournament_type`),
  CONSTRAINT `fk_table1_hosting_tournament1`
    FOREIGN KEY (`tournament_date` , `billiard_club_id` , `tournament_type`)
    REFERENCES `billiard_db`.`hosting_tournament` (`date` , `billiard_club_id` , `tournament_type`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
USE `billiard_db`$$
DROP TRIGGER IF EXISTS `billiard_db`.`new_match_BEFORE_INSERT` $$
USE `billiard_db`$$
CREATE DEFINER = CURRENT_USER TRIGGER `billiard_db`.`new_match_BEFORE_INSERT` BEFORE INSERT ON `new_match` FOR EACH ROW
BEGIN
	IF (NEW.finished = TRUE) THEN
		IF (NEW.score1 > NEW.score2) THEN
			SET NEW.winner_id = NEW.id_player1;
		ELSE
			SET NEW.winner_id = NEW.id_player2;
		END IF;
	END IF;
END$$
...
USE `billiard_db`$$
DROP TRIGGER IF EXISTS `billiard_db`.`new_match_BEFORE_UPDATE` $$
USE `billiard_db`$$
CREATE DEFINER = CURRENT_USER TRIGGER `billiard_db`.`new_match_BEFORE_UPDATE` BEFORE UPDATE ON `new_match` FOR EACH ROW
BEGIN
	IF (NEW.finished = TRUE) THEN
		IF (NEW.score1 > NEW.score2) THEN
			SET NEW.winner_id = NEW.id_player1;
...
INSERT INTO `billiard_db`.`player` (`id`, `name`, `last_name`, `email`, `phone`, `img_link`) VALUES (DEFAULT, 'Uros', 'Urosevic', 'uros.urosevic@hotmail.com', '+3812349823', '../images/players/placeholder.jpg');
INSERT INTO `billiard_db`.`player` (`id`, `name`, `last_name`, `email`, `phone`, `img_link`) VALUES (DEFAULT, 'Milan', 'Milanovic', 'milan.milanovic@ptt.rs', '+3812312313', '../images/players/placeholder.jpg');
INSERT INTO `billiard_db`.`player` (`id`, `name`, `last_name`, `email`, `phone`, `img_link`) VALUES (DEFAULT, 'Dusan', 'Dusanovic', 'dusan.dusanovic@yandex.com', '+3812523526', '../images/players/placeholder.jpg');
INSERT INTO `billiard_db`.`player` (`id`, `name`, `last_name`, `email`, `phone`, `img_link`) VALUES (DEFAULT, 'Janko', 'Jankovic', 'janko.jankovic@krstarica.com', '+3812342342', '../images/players/placeholder.jpg');
...
START TRANSACTION;
USE `billiard_db`;
INSERT INTO `billiard_db`.`billiard_club` (`id`, `name`, `username`, `password`, `img_link`, `email`, `phone`, `city`, `address`, `zip_code`, `num_of_tables`) VALUES (DEFAULT, 'Two table club', 'two_table_club', '123456', NULL, 'two_table_club@gmail.com', '+3817584930', 'Belgrade', 'Majke Jevrosime 19', '11000', 2);
INSERT INTO `billiard_db`.`billiard_club` (`id`, `name`, `username`, `password`, `img_link`, `email`, `phone`, `city`, `address`, `zip_code`, `num_of_tables`) VALUES (DEFAULT, 'One table club', 'one_table_club', '123456', NULL, 'one_table_club@gmail.com', '+3813242342', 'Belgrade', 'Majke Jevrosime 20', '11000', 1);
INSERT INTO `billiard_db`.`billiard_club` (`id`, `name`, `username`, `password`, `img_link`, `email`, `phone`, `city`, `address`, `zip_code`, `num_of_tables`) VALUES (DEFAULT, 'Three table club', 'three_table_club', '123456', '', 'three_table_club@gmail.com', '+3814532332', 'Belgrade', 'Majke Jevrosime 21', '11000', 3);
...
START TRANSACTION;
USE `billiard_db`;
INSERT INTO `billiard_db`.`hosting_tournament` (`date`, `billiard_club_id`, `name`, `entry_fee`, `prize`, `tournament_type`, `tournament_key`, `active`, `schema_JSON`) VALUES ('NOW()', 1, 'bla bla', 0, 0, 'Drzavni', 'proba123', true, NULL);
