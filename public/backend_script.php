<?php
	...
	switch ($table_message)
	{
		...
		case 'is_match_ready':
		{
			...
			$query = $db->prepare("SELECT player_id FROM playing_tournament WHERE tournament_date = :td AND billiard_club_id = :bci AND tournament_type = :tt AND active = false ORDER BY next_round ASC");
